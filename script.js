...
window.askNewKey = () => {
    const key = prompt("🔑 Clé API Gemini (AIza...) :");
...
window.handleFileUpload = async (e) => {
    ...
    } catch (err) { alert("Erreur lecture document"); }
};
...
window.processCourse = async () => {
    ...
    const promptText = `Analyse ce cours. Réponds en JSON : {"titre":"", "sections":[{"n":"", "c":""}], "quiz":[{"q":"", "correct":"", "wrong":[]}]}. Texte : ${extractedText.substring(0, 20000)}`;
    ...
    } catch (err) { clearInterval(interval); alert("Erreur Gemini : " + err.message); document.getElementById('btn-ai').classList.remove('hidden'); }
};
...
window.renderHistory = () => {
    ...
    filtered.forEach(c => {
        ...
        d.style = "background:#0f172a; padding:8px; border-radius:8px; display:flex; justify-content:space-between; align-items:center; border-left:3px solid #6366f1;";
        d.innerHTML = `<span onclick="viewCourse(${c.id})" style="flex:1; cursor:pointer; font-size:0.8em; color:white;">📄 ${c.name.substring(0,15)}</span>
                       <i class="fas fa-trash-alt" onclick="deleteCourse(${c.id})" style="color:#ef4444; padding:5px;"></i>`;
        list.appendChild(d);
    });
};

window.deleteCourse = (id) => { if(confirm("Supprimer ?")) { dbCourses = dbCourses.filter(c => c.id !== id); localStorage.setItem('ai_studiant_db', JSON.stringify(dbCourses)); renderHistory(); } };
window.viewCourse = (id) => { const c = dbCourses.find(i => i.id === id); if(c) { renderResults(c.result); document.getElementById('results-container').classList.remove('hidden'); } };
